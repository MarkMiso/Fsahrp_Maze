...
type CharInfo with
    ...
    /// Shortcut for creating a wall cell given the foregraound and optional background colors.
...
type wronly_raster (w, h) =
    ...
    member this.draw_maze (m : maze, w, h, px) =
        ...
        for y in 0..((h / 2) - 1) do
            for x in 0..((w / 2) - 1) do
                if (m.cell(x, y).corner) then plot (x + j) (y + i)
                if (m.cell(x, y).wall_up) then plot (x + j + 1) (y + i)
                if (m.cell(x, y).wall_left) then plot (x + j) (y + i + 1)
                if (m.cell(x, y).room) then plot (x + j + 1) (y + i + 1)
                j <- j + 1
            j <- 0
            i <- i + 1

        this.draw_line (w, 0, w, h, px) 
